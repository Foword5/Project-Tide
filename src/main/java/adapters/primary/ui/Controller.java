...
import adapters.primary.ui.vu.BoutonDate;
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getActionCommand() == "jour") {
			...
			if(port == null) {
				...
				MareeJour mareeDomain = portUseCase.getMareejour(date.getJour(), date.getMois(), port);
				UiMareeJour maree = new UiMareeJour(
					mareeDomain.getHeuresPleineMer(),
					mareeDomain.getHauteursPleineMer(),
					mareeDomain.getHeuresBasseMer(),
					mareeDomain.getHauteursBasseMer(),
					mareeDomain.getCoefficients(),
					mareeDomain.getHauteurs()
				);
				panelAffichage.getPanelPleineMer().setValue(maree);
				panelAffichage.getPanelHeureParHeure().setValue(maree);

				panelAffichage.getPanelPleineMer().getJourMois().setText(port + " : " + date.toString());
				panelAffichage.getPanelHeureParHeure().getJourMois().setText(port + " : " + date.toString());
			}
			return;
		}
			...
			MareeJour mareeDomain = portUseCase.getMareejour(date.getJour(), date.getMois(), port);
			String[] hauteurs = mareeDomain.getHauteurs();
